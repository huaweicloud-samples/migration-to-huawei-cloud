...
- Precheck status handling: If `Unavailable`, leave `Recommended Spec` empty unless official documentation confirms a supported alternative in the same target region. Record the service code, target region, and precheck status (`Available`, `Unavailable`, `Pending Confirmation`, or `Skipped`) in `Recommendation Notes` for every checked row. Do not treat `Pending Confirmation` as available.
